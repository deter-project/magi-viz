...
try {
  ...
  // access database
  $db = $conn->magi;
  $collection = $db->pktcounter_u;

  $first = $collection->find(array("port" => "out-node-1"), array("created" => 1, "bytes" => 1))->sort(array("created" => 1))->limit(1);
  ...
  //echo $newts;
  $cursor = $collection->find(array("created" => array('$gt' => $newts),"port" => "out-node-1"), array("created" => 1, "bytes" => 1))->sort(array("created" => 1))->limit(1);
